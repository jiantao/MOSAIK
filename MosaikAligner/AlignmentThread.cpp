...
void CAlignmentThread::AlignReadArchive(MosaikReadFormat::CReadReader* pIn, MosaikReadFormat::CAlignmentWriter* pOut, FILE* pUnalignedStream, uint64_t* pReadCounter, bool isPairedEnd) {
        ...
        while(true) {
                ...
                // local alignment flag is turned on and not both of mates are uniquely aligned.
                if(areBothMatesPresent && mFlags.UseLocalAlignmentSearch) {

                        // perform local alignment search WHEN MATE1 IS UNIQUE AND MATE2 IS NON-UNIQUE OR NOT ALIGNED
                        ...
                        // perform local alignment search WHEN MATE2 IS UNIQUE AND MATE1 IS NON-UNIQUE OR NOT ALIGNED
                        if(!isMate1Unique && isMate2Unique) {
...
bool CAlignmentThread::RescueMate(const LocalAlignmentModel& lam, const CMosaikString& bases, const unsigned int uniqueBegin, const unsigned int uniqueEnd, const unsigned int refIndex, Alignment& al) 
{
        ...
        // handle the case that the read is on reverse strand
